...
Klasse:     Moorgeck
            Implementiert die Eigenschaften und das Verhalten des Monsters Moorgeck
...
Moorgeck::Moorgeck(const string& n)
{
    name=n;
    typ="Wasser";
    level=3;
...
Moorgeck::Moorgeck(const string& n, const int& l, const int& h, const int& hm, const int& e, const int& em, const bool& k, const int& angr, const int& vert, const int& init, const int& ap1, const int& ap2 )
{
    name=n;
    typ="Wasser";
    level=l;
...
/*
 * Werte von Moorgeck als string zurückgeben
 * @return: Werte des Moorgecks als string
*/
string Moorgeck::statistik() const{
    string s="";
...
*/
tuple<int,string> Moorgeck::angreifen(const int& i) const{

    typedef tuple<int,string> ang;

    if(!kampfbereit){//wenn das Moorgeck nicht kampfbereit ist, kann es nicht angreifen
        ang a(0,attacke[i]->get_typ());
        cout<<"Moorgeck nicht Kampfbereit"<<endl;
        return a;
    }

    attacke[i]->ausfuehren();//ausgewählte attacke wird ausgeführt

    if(attacke[i]->get_typ()==typ){//Wenn attacke gleichen Typ wie Moorgeck hat (Wasser), dann erhöhen sich die Schadenspunkte um einen Bonus
...
/*
 * Moorgeck wird angegriffen
 * @param ang: ein tupel aus Schadenspunkte (int), Typ der Attacke (string)
 * @return: EP werden zurückgegeben
*/
int Moorgeck::wird_angegriffen(const tuple<int,string>& ang){
    if(!kampfbereit){//Wenn Moorgeck nicht kampfbereit ist, kann es nicht angegriffen werden
    ...
    if(t==typ){//Wenn der Typ der Attacke der gleiche wie von Moorgeck (Wasser) ist, dann reduzieren sich die Schadenspunkte
        w=w*0.7;
    }
    ...
    if(hp<=0){//wenn HP unter 0 fallen
        ...
        kampfbereit=false;//Moorgeck ist nicht mehr Kampfbereit
 ...
 * @param punkte: erfahrungspunkte die Moorgeck durch einen Angriff erhält
*/
void Moorgeck::leveln(const int& punkte){
...
/*
 * Moorgeck Kampfbereit setzen
*/
void Moorgeck::heilen(){
    hp=hp_max; //werte werden auf max werte gesetzt
    attacke[0]->set_ap();
    attacke[1]->set_ap();

    kampfbereit=true;//Moorgeck ist kampfbereit
}
 ...
 * @return: name
*/
string Moorgeck::get_name() const{
    return name;
}
 ...
 * @return: ep_max
*/
int Moorgeck::get_epMax() const{
    return ep_max;
}

/*
 * zurückgeben, ob Moorgeck kampffähig ist
 * @return: status kampfbereit
*/
bool Moorgeck::ist_kampfbereit() const{
    return kampfbereit;
}
...
*/
Attacke* Moorgeck::get_Attacke1() const{
    return attacke[0];
}
...
*/
Attacke* Moorgeck::get_Attacke2() const{
    return attacke[1];
}
...
*/
int Moorgeck::get_angriff() const{
    return angriff;
}
...
*/
int Moorgeck::get_verteidigung() const{
    return verteidigung;
}
 ...
 * @return: initiative
*/
int Moorgeck::get_initiative() const{
    return initiative;
}
