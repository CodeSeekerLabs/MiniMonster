...
Klasse:     Floradon
            Implementiert die Eigenschaften und das Verhalten des Monsters Floradon
...
Floradon::Floradon(const string& n)
{
    name=n;
    typ="Pflanze";
    level=3;
...
Floradon::Floradon(const string& n, const int& l, const int& h, const int& hm, const int& e, const int& em, const bool& k, const int& angr, const int& vert, const int& init, const int& ap1, const int& ap2 )
{
    name=n;
    typ="Pflanze";
    level=l;
...
/*
 * Werte von Floradon als string zurückgeben
 * @return: Werte des Floradon als string
*/
string Floradon::statistik() const{
    string s="";
...
*/
tuple<int,string> Floradon::angreifen(const int& i) const{

    typedef tuple<int,string> ang;

    if(!kampfbereit){//wenn das Floradon nicht kampfbereit ist, kann es nicht angreifen
        ang a(0,attacke[i]->get_typ());
        cout<<"Floradon nicht Kampfbereit"<<endl;
        return a;
    }

    attacke[i]->ausfuehren();//ausgewählte attacke wird ausgeführt

    if(attacke[i]->get_typ()==typ){//Wenn attacke gleichen Typ wie Floradon hat (Wasser), dann erhöhen sich die Schadenspunkte um einen Bonus
...
/*
 * Floradon wird angegriffen
 * @param ang: ein tupel aus Schadenspunkte (int), Typ der Attacke (string)
 * @return: EP werden zurückgegeben
*/
int Floradon::wird_angegriffen(const tuple<int,string>& ang){
    if(!kampfbereit){//Wenn Floradon nicht kampfbereit ist, kann es nicht angegriffen werden
    ...
    if(t==typ){//Wenn der Typ der Attacke der gleiche wie von Floradon (Pflanze) ist, dann reduzieren sich die Schadenspunkte
        w=w*0.7;
    }
    ...
    if(hp<=0){//wenn HP unter 0 fallen
        ...
        kampfbereit=false;//Floradon ist nicht mehr Kampfbereit
 ...
 * @param punkte: erfahrungspunkte die Floradon durch einen Angriff erhält
*/
void Floradon::leveln(const int& punkte){
...
/*
 * Floradon Kampfbereit setzen
*/
void Floradon::heilen(){
    hp=hp_max; //werte werden auf max werte gesetzt
    attacke[0]->set_ap();
    attacke[1]->set_ap();

    kampfbereit=true;//Floradon ist kampfbereit
}
 ...
 * @return: name
*/
string Floradon::get_name() const{
    return name;
}
 ...
 * @return: ep_max
*/
int Floradon::get_epMax() const{
    return ep_max;
}

/*
 * zurückgeben, ob Floradon kampffähig ist
 * @return: status kampfbereit
*/
bool Floradon::ist_kampfbereit() const{
    return kampfbereit;
}
...
*/
Attacke* Floradon::get_Attacke1() const{
    return attacke[0];
}
...
*/
Attacke* Floradon::get_Attacke2() const{
    return attacke[1];
}
...
*/
int Floradon::get_angriff() const{
    return angriff;
}
...
*/
int Floradon::get_verteidigung() const{
    return verteidigung;
}
 ...
 * @return: initiative
*/
int Floradon::get_initiative() const{
    return initiative;
}
